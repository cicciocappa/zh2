...
# applicazione host degli assi catalogo (ENTITY_DESIGN §6+§8.5)
test_prop_world: test_prop_world.c prop_world.c prop_world.h props.c scene.c \
...
# richiamo da fuoco: le torrette che sparano attirano l'orda
test_lure: test_lure.c sim_particles.c defense.c defense.h
	$(CC) $(CFLAGS) -o $@ test_lure.c sim_particles.c defense.c $(LDLIBS)

# economia biomassa (BIOMASS_DESIGN): convertitore (modulo bio, zero-dep)
test_bio: test_bio.c bio.c bio.h
	$(CC) $(CFLAGS) -o $@ test_bio.c bio.c $(LDLIBS)

# caricatori torretta (BIOMASS_DESIGN §5): mag/reload + guardia legacy
...
# soldato giocabile (SOLDIER_DESIGN, LOOP_DESIGN F): modulo zero-dep sul core
test_soldier: test_soldier.c soldier.c soldier.h sim_particles.c
	$(CC) $(CFLAGS) -o $@ test_soldier.c soldier.c sim_particles.c $(LDLIBS)
